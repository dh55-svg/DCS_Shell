...
public:
    ...
    ApplicationBuilder& withPluginHub(PluginHub* hub) { m_pluginHub = hub; return *this; }
    ...
    ApplicationBuilder& withFieldbus() {
        if (m_pluginHub) {
            ...
            if (fb) {
                m_ctx->fieldbus = std::shared_ptr<IFieldbus>(fb, [](IFieldbus*){});
                return *this;
            }
        }
    ...
    ApplicationBuilder& withDatabase() {
        if (m_pluginHub) {
            ...
            if (alarm && history) {
                m_ctx->alarmRepo = std::shared_ptr<IAlarmRepo>(alarm, [](IAlarmRepo*){});
                m_ctx->historyRepo = std::shared_ptr<IHistoryRepo>(history, [](IHistoryRepo*){});

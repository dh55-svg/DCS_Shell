...
public:
    ...
    using SnapshotMap = std::unordered_map<quint32, Snapshot>;
    ...
    void write(quint32 tagId, const Snapshot& snapshot) {
        m_writeBuffer[tagId] = snapshot;
    }

    void writeBatch(const std::vector<Snapshot>& snapshots) {
    ...
    Snapshot readTag(quint32 tagId) const {
        auto it = readAll();

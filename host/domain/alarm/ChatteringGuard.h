#ifndef CHATTERINGGUARD_H
#define CHATTERINGGUARD_H
#include <QHash>
#include <QDateTime>
...
public:
    bool check(quint32 tagId, int maxRepeatsPerMin = 3) {
        ...
        if (st.windowStart == 0 || (now - st.windowStart) > 60000) {
            st.count = 1; st.windowStart = now; return false;
        }
        st.count++;
        if (st.count >= maxRepeatsPerMin) { st.autoShelved = true; return true; }
        return false;
    }
...
private:
    struct State { int count = 0; qint64 windowStart = 0; bool autoShelved = false; };

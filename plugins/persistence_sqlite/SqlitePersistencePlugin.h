#ifndef SQLITEPERSISTENCEPLUGIN_H
#define SQLITEPERSISTENCEPLUGIN_H
#include <QObject>
...
class SqlitePersistencePlugin : public QObject,
    public IAlarmRepo,
    public IHistoryRepo,
    public ITagRepo,
    public IOperationRepo {
    Q_OBJECT
    Q_PLUGIN_METADATA(IID "com.dcsshell.SqlitePersistence" FILE "SqlitePersistencePlugin.json")
    // IAlarmRepo/IHistoryRepo/ITagRepo/IOperationRepo are non-QObject interfaces
    // — they are resolved via dynamic_cast in PluginHub

public:
    explicit SqlitePersistencePlugin(QObject* parent = nullptr);
    ~SqlitePersistencePlugin() override;

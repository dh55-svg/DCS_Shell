...
#include "../messaging/IMessageBus.h"
#include <QThread>
#include <QDebug>
ModbusImpl::ModbusImpl(QObject *parent)
    : IFieldbus{parent}
{}
...
void ModbusImpl::removeDevice(int devId) {
    for (int i = 0; i < m_devices.size(); ++i) {
        if (m_devices[i].config.deviceId == devId) {
            ...
            if (dev.comm) {
                ...
                dev.comm->deleteLater();
            }
            if (dev.thread) {
                dev.thread->quit();
                dev.thread->wait(3000);
            }
...
void ModbusImpl::startDevice(int devId) {
    for (auto& dev : m_devices) {
        if (dev.config.deviceId == devId && dev.comm && dev.comm->isConnected()) {
            QMetaObject::invokeMethod(dev.comm, "startPoll", Qt::QueuedConnection);
            return;
        }
    }
}
void ModbusImpl::stopDevice(int devId) {
    for (auto& dev : m_devices) {
